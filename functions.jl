...
function get_sorted_faculties_id_list(faculties_list, student)#alpha...学部の文系度合い, 学部の理系度合い（それぞれ[0, 1])の値を取る
    ...
    sorted_faculties_id_list[1:length(available_faculties_list)] = map(f -> f.id, sort(available_faculties_list, by = f -> -abs(f.alpha-student.score_science)-abs(f.beta-student.score_liberal_arts)))#sort関数は適当
